...
describe("FurnitureContent Component", () => {
  it("When using the search bar, typing a specific term like “Table” for instance only brings up items with “table” in its name or description.", async () => {
    ...
    if (furnitureItems.length > 0) {
      ...
      const firstItemDescriptionElement = firstItem.querySelector(
        ".furniture-item-description"
      );
      const regex = /table/i;
      const titleIncludesTable = regex.test(firstItemTitleElement.textContent);
      const descriptionIncludesTable = regex.test(
        firstItemDescriptionElement.textContent
      );
      expect(titleIncludesTable || descriptionIncludesTable).toBe(true);
    }
  });

  it("fails to find 'table' in the name or description of furniture items when it is not present", async () => {
    const itemMockWithoutTable = {
      ...itemMock,
      items: itemMock.items.map((item) => ({
        ...item,
        furnitureName: item.furnitureName.replace(/table/gi, ""),
        description: item.description.replace(/table/gi, ""),
      })),
    };

    const { container } = render(
      <BrowserRouter>
        <FurnitureContent data={itemMockWithoutTable} />
      </BrowserRouter>
    );
    const input = screen.getByRole("textbox");
    fireEvent.change(input, { target: { value: "table" } });
    ...
    if (furnitureItems.length === 0) {
      ...
      expect(titleIncludesTable || descriptionIncludesTable).toBe(false);
    }
  });
});
